...
def combine_images_in_directory(dir_path):
    """
    주어진 디렉토리 내 이미지를 합쳐서 하나의 이미지를 저장하는 함수.
    ...
    # 첫 번째 이미지의 크기 가져오기 (모든 이미지가 같은 크기라고 가정)
    if not images:  # 이미지가 없는 경우 예외 처리
        raise ValueError("디렉토리에 이미지가 없습니다.")
    ...
    # 결과 이미지 저장
    combined_image.save(os.path.join(dir_path, "combined_image_all.png"))

    print("모든 이미지를 합쳐서 combined_image_all.png로 저장하였습니다.")
